...
export function clearDiagnosticsForClosedFile(
  diagnosticCollection: vscode.DiagnosticCollection,
...
function parseClangOutput(
  ...
  for (const line of lines) {
    ...
    if (match) {
      const [, filePathWithChunk, chunkIndexStr, lineStr, columnStr, message] =
        match;
      const chunkIndex = parseInt(chunkIndexStr, 10);
      const adjustedLineNumber =
        parseInt(lineStr, 10) - 1 + chunkSize * chunkIndex; // Adjust line number
